...
async function runStep(params: {
  ...
  const contextSummary =
    priorArtifacts.length > 0
      ? priorArtifacts.map(a => `- ${a.filename}`).join("\n")
      : "None yet.";
  ...
  try { json = match ? JSON.parse(match[0]) : {}; } catch { json = {}; }
...
export async function runBMAD(args: RunBMADArgs): Promise<BMADOutput> {
  ...
  try {
    ...
    // If BMAD files are not available, return a simple fallback
    if (e.code === 'ENOENT') {
      const fallbackResult = {
        plan: ["Break down task", "Map deliverables", "Assemble artifacts", "Deliver"],
        summary: `BMAD completed: ${objective} (using fallback mode - BMAD files not found)`,
        artifacts: [{ 
          filename: "BMAD_SUMMARY.md", 
          content: `# BMAD Summary\n\nObjective: ${objective}\n\nStatus: Completed using fallback mode.\n\nNote: To use full BMAD functionality, please add the BMAD vendor files to agents/vendor/bmad/`
        }]
      };
